...
import { ArrowUp, ArrowDown, MapPin, ExternalLink } from "lucide-react";
...
export function SponsoredProviderCard({ provider, onEnquire, className = "" }: SponsoredProviderCardProps) {
  ...
  return (
    <div
      className={`bg-white border-4 border-amber-400 rounded-3xl shadow-md hover:border-amber-500 hover:shadow-lg transition-all duration-200 overflow-hidden flex flex-col justify-between relative ${className}`}
      id={`sponsored-prov-card-${provider.id}`}
    >
      {/* SPONSOR SPOTLIGHT TOP HEADER */}
      <div className="bg-amber-400 text-slate-905 px-4 py-1.5 flex justify-between items-center gap-2 text-xs font-black tracking-wider uppercase shadow-xs">
        <span>Featured Wiltshire Sponsor</span>
        <span className="font-black bg-white/40 text-[9px] px-1.5 rounded">Featured</span>
      </div>
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex justify-between items-start gap-4">
          ...
          <div className="text-right shrink-0">
            <span className="inline-block text-[10px] font-bold text-teal-850 bg-teal-50 px-2 py-0.5 rounded border border-teal-200">
              {provider.bestFor}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 bg-slate-50 rounded-xl p-3 border border-slate-200">
          ...
          <div className="flex items-center gap-2">
            <div className="p-1.5 bg-amber-55 rounded-md text-amber-900 shrink-0 border border-amber-200">
